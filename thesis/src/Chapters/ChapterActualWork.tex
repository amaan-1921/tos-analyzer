...
The method consists of document ingestion\index{Document Ingestion} where users upload Terms of
...
Next, semantic embeddings\index{Semantic Embedding} are created for each segmented text unit
...
Alongside embedding generation, language models\index{LLM} can also be used to
extract subject-predicate-object triples\index{Triple Extraction} from textual segments, which
in turn form the groundwork for a representation in a knowledge
graph\index{Knowledge Graph}. Such a structured representation can account for relational
...
structure, a hybrid retrieval mechanism\index{Hybrid Retrieval} is designed to use vector
similarity search\index{Vector Similarity Search} together with knowledge graph traversal\index{Graph Traversal}. This
...
traversal\index{Graph Traversal} results were checked to confirm that relational context
contributed meaningful information beyond vector retrieval results.
The risk analysis\index{Risk Analysis} outputs created by the reasoning module were
...
processing modules: text extraction, semantic chunking\index{Semantic Chunking}, embedding
...
utilizes a graph database\index{Graph Database} that has vector indexing and relational
