...
\subsection{Document Ingestion and Semantic Parsing}
...
chunks. Semantic chunking, as opposed to fixed-length segmentation
...
Semantic embeddings are obtained for each segment of the text chunk,
...
similarity-based retrieval during query processing. The vector
...
graph for the document. Large Language Models are used to derive
subject-predicate-object triples that describe relationships between
...
a graph database, preserving relational and logical dependencies
within the document.

\subsection{Hybrid Retrieval Mechanism}
In the system we implement a hybrid retrieval strategy involving
vector similarity and knowledge graph traversal. Vector retrieval
...
A generative language model set up as a legal risk assessor gets the
